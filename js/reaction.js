/* ═══════════════════════════════════════════════════════════════
   ...
   ─ Green → tap/click  ─ Red → do NOT react  ─ Yellow → key press
   ═══════════════════════════════════════════════════════════════ */
...
const ReactionModule = (() => {
  ...
  function reset() {
    state = {
      ...
      attempts: 0, correct: 0, rts: [], bestRT: null,
      signalActive: false, signalType: null, signalKey: null,
      signalStart: 0, falseTaps: 0,
    };
  }

  const SIGNALS = [
    { type:'green',  emoji:'🟢', instruction:'CLICK / TAP — GREEN SIGNAL!', color:'#00cc66', action:'click' },
    { type:'red',    emoji:'🔴', instruction:'DO NOT REACT — RED SIGNAL!',  color:'#ff3c3c', action:'none'  },
    { type:'yellow', emoji:'🟡', instruction:'PRESS THE KEY: ',             color:'#ffd600', action:'key'   },
    { type:'blue',   emoji:'🔵', instruction:'DOUBLE-TAP / DOUBLE-CLICK!',  color:'#00aaff', action:'dblclick' },
  ];
  ...
  function showSignal() {
    ...
    state.signalActive = false; state.signalType = null;
    ...
    signalTimeout = setTimeout(() => {
      ...
      state.signalType = sig.type; state.signalKey = key;
      state.signalActive = true; state.signalStart = Date.now(); state.attempts++;
      const shapeEl=$id('rt-shape'), instrEl=$id('rt-instruction');
      shapeEl.textContent = sig.emoji;
      shapeEl.style.background = sig.color + '33';
      shapeEl.style.border = `4px solid ${sig.color}`;
      instrEl.textContent = sig.action === 'key' ? sig.instruction + key : sig.instruction;
      ...
      updateStats();
      if (sig.type !== 'red') {
        clearSignalTimeout = setTimeout(() => { if (state.signalActive && state.running) missSignal(); }, 2000);
      } else {
        clearSignalTimeout = setTimeout(() => {
          if (state.signalActive && state.running) {
            state.correct++; state.signalActive = false;
            showFeedback('✓ CORRECT — NO REACT','#00ff88');
            ADAPTAudio.playCorrect(); updateStats();
  ...
  function handleReact(isDouble = false) {
    ...
    if (!state.signalActive) {
      ...
      ADAPTAudio.playWrong(); return;
    }
    clearTimeout(clearSignalTimeout);
    const rt = Date.now() - state.signalStart;
    if (state.signalType === 'red') {
      state.signalActive = false;
      $id('rt-signal').classList.add('hidden'); $id('rt-waiting').classList.remove('hidden');
      showFeedback("✗ WRONG — DON'T TAP RED!",'#ff3c3c');
      ADAPTAudio.playWrong(); updateStats();
      setTimeout(() => showSignal(), 800); return;
    }
    if (state.signalType === 'blue' && !isDouble) return;
    state.correct++; state.rts.push(rt);
  ...
  function handleKeyPress(key) {
    if (!state.running || !state.signalActive || state.signalType !== 'yellow') return;
  ...
  function showFeedback(msg, color) {
    ...
    if (!fb) return;
    fb.textContent = msg; fb.style.color = color;
    fb.className = 'rt-feedback'; fb.classList.remove('hidden');
    clearTimeout(fb._t);
    fb._t = setTimeout(() => fb.classList.add('hidden'), 800);
  }

  function updateStats() {
    const avgRT = state.rts.length > 0 ? Math.round(state.rts.reduce((a,b)=>a+b,0)/state.rts.length) : null;
    $id('rt-attempts').textContent = state.attempts;
    $id('rt-correct').textContent  = state.correct;
    $id('rt-avg').textContent      = avgRT ? avgRT+'ms' : '—';
    $id('rt-best').textContent     = state.bestRT ? state.bestRT+'ms' : '—';
  }
  ...
  function startCountdown() {
    countdownInterval = setInterval(() => {
      ...
      state.timeLeft--;
      const el=$id('rt-timer'), m=Math.floor(state.timeLeft/60), s=state.timeLeft%60;
  ...
  function start() {
    ...
    showSignal(); startCountdown();
  }

  function endTest() {
    state.running=false;
    clearTimeout(signalTimeout); clearTimeout(clearSignalTimeout);
    ...
    const avgRT=state.rts.length>0?Math.round(state.rts.reduce((a,b)=>a+b,0)/state.rts.length):null;
    ADAPTStorage.addScore('reaction',{accuracy:Math.round(accuracy),avgRT,bestRT:state.bestRT,attempts:state.attempts,correct:state.correct,falseTaps:state.falseTaps});
    const rEl=$id('rt-result'), color=accuracy>=80?'#00ff88':accuracy>=60?'#ffd600':'#ff3c3c';
    rEl.classList.remove('hidden');
    rEl.innerHTML=`
      <div class="result-score" style="color:${color}">${accuracy.toFixed(0)}%</div>
      <div class="result-label">Reaction Accuracy</div>
      <div class="result-breakdown">
        <div class="result-item"><div class="r-label">Best RT</div><div class="r-val">${state.bestRT?state.bestRT+'ms':'—'}</div></div>
  ...
  function init(){ $id('rt-start-btn').addEventListener('click', start); }
  return { init };
})();
