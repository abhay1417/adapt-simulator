/* ═══════════════════════════════════════════════════════════════
   ...
   Simulate cockpit gauges: altitude, airspeed, heading, vspeed
   Random anomalies appear – user must click the anomalous gauge
   ═══════════════════════════════════════════════════════════════ */
...
const MonitoringModule = (() => {
  ...
  const INSTRUMENTS = {
    altitude: {
      min: 0, max: 15000,
      normalMin: 2000, normalMax: 8000,
      unit: 'ft', label: 'ALTITUDE',
      current: 5000, target: 5000, velocity: 0,
      canvas: 'canvas-altitude', valueEl: 'val-altitude', flashEl: 'flash-altitude',
      gaugeEl: 'gauge-altitude',
      color: '#00aaff',
      format: v => Math.round(v).toLocaleString() + ' ft',
    },
    airspeed: {
      min: 0, max: 400,
      normalMin: 120, normalMax: 280,
      unit: 'kts', label: 'AIRSPEED',
      current: 200, target: 200, velocity: 0,
      canvas: 'canvas-airspeed', valueEl: 'val-airspeed', flashEl: 'flash-airspeed',
      gaugeEl: 'gauge-airspeed',
      color: '#ff8800',
      format: v => Math.round(v) + ' kts',
    },
    heading: {
      min: 0, max: 360,
      normalMin: null, normalMax: null, // always normal
      unit: '°', label: 'HEADING',
      current: 90, target: 90, velocity: 0,
      canvas: 'canvas-heading', valueEl: 'val-heading', flashEl: 'flash-heading',
      gaugeEl: 'gauge-heading',
      color: '#aa44ff',
      format: v => Math.round(v).toString().padStart(3,'0') + '°',
    },
    vspeed: {
      min: -2000, max: 2000,
      normalMin: -500, normalMax: 500,
      unit: 'fpm', label: 'VERT SPEED',
      current: 0, target: 0, velocity: 0,
      canvas: 'canvas-vspeed', valueEl: 'val-vspeed', flashEl: 'flash-vspeed',
      gaugeEl: 'gauge-vspeed',
      color: '#00ff88',
      format: v => (v >= 0 ? '+' : '') + Math.round(v) + ' fpm',
    }
  };
  ...
  function reset() {
    state = {
      ...
      anomaliesCaught: 0,
      anomaliesMissed: 0,
      activeAnomalies: {}, // key = instrument, value = { startTime, caught }
      activeWarnings: {},  // key = warnId, value = { startTime, caught }
      catchTimes: [],
    };
    // Reset instruments
    Object.values(INSTRUMENTS).forEach(inst => {
      inst.current = (inst.normalMin !== null) ? (inst.normalMin + inst.normalMax) / 2 : 90;
      inst.target = inst.current;
      inst.velocity = 0;
    });
  }
  ...
  function drawGauge(name) {
    ...
    const textCol = isDark ? '#e8f0ff' : '#0a1530';
    const borderCol = isDark ? '#1e3050' : '#aabbd8';
    ...
    // Normal zone (green arc)
    if (inst.normalMin !== null) {
      const nStart = startAngle + ((inst.normalMin - inst.min) / range) * totalAngle;
      const nEnd = startAngle + ((inst.normalMax - inst.min) / range) * totalAngle;
      ctx.beginPath();
      ctx.arc(cx, cy, R - 6, nStart, nEnd);
    ...
    for (let i = 0; i <= 10; i++) {
      ...
      ctx.strokeStyle = textCol; ctx.lineWidth = i % 5 === 0 ? 2 : 1;
      ctx.stroke();
    }

    // Needle
    const norm = (inst.current - inst.min) / range;
    const needleAngle = startAngle + norm * totalAngle;
    const isAnomaly = name !== 'heading' && inst.normalMin !== null &&
      (inst.current < inst.normalMin || inst.current > inst.normalMax);
    const needleColor = isAnomaly ? '#ff3c3c' : inst.color;
    ...
    // For heading: draw compass letters
    if (name === 'heading') {
      ...
      Object.entries(letters).forEach(([deg, letter]) => {
        const a = startAngle + (deg / 360) * totalAngle;
        const lr = R - 22;
    ...
    const valEl = $id(inst.valueEl);
    if (valEl) valEl.textContent = inst.format(inst.current);
  }

  function drawAllGauges() {
    Object.keys(INSTRUMENTS).forEach(drawGauge);
  }
  ...
  function triggerAnomaly() {
    ...
    const type = Math.random() < 0.6 ? 'gauge' : 'warning';

    if (type === 'gauge') {
      // Pick gauge (not heading, not already anomalous)
      const candidates = Object.keys(INSTRUMENTS).filter(n => {
        const inst = INSTRUMENTS[n];
        return inst.normalMin !== null && !state.activeAnomalies[n];
      });
      if (!candidates.length) { scheduleAnomaly(); return; }

      const name = candidates[Math.floor(Math.random() * candidates.length)];
      const inst = INSTRUMENTS[name];
      // Push target out of range
      const above = Math.random() < 0.5;
      if (above) {
        inst.target = inst.normalMax + (inst.max - inst.normalMax) * (0.3 + Math.random() * 0.5);
      } else {
        inst.target = inst.normalMin - (inst.normalMin - inst.min) * (0.3 + Math.random() * 0.5);
      }

      state.anomaliesTotal++;
      state.activeAnomalies[name] = { startTime: Date.now(), caught: false };
      ...
      setTimeout(() => {
        if (state.activeAnomalies[name] && !state.activeAnomalies[name].caught && state.running) {
      ...
      // Warning light
      const freeSlots = WARNING_LIGHTS.filter(w => !state.activeWarnings[w]);
      if (!freeSlots.length) { scheduleAnomaly(); return; }
      const warnId = freeSlots[Math.floor(Math.random() * freeSlots.length)];
      state.anomaliesTotal++;
      state.activeWarnings[warnId] = { startTime: Date.now(), caught: false };
      ...
      setTimeout(() => {
        if (state.activeWarnings[warnId] && !state.activeWarnings[warnId].caught && state.running) {
          state.anomaliesMissed++;
          clearWarning(warnId);
          updateStats();
        }
  ...
  function clearAnomaly(name) {
    ...
    $id(inst.gaugeEl).classList.remove('anomaly');
    $id(inst.gaugeEl).classList.remove('clicked-ok');
  }

  function clearWarning(warnId) {
    delete state.activeWarnings[warnId];
    const lightEl = document.querySelector(`.warn-light[data-warn="${warnId}"]`);
    if (lightEl) { lightEl.classList.remove('active'); lightEl.classList.remove('acknowledged'); }
  }

  /* ── Click handling ── */
  function handleGaugeClick(name) {
    if (!state.running) return;
    if (state.activeAnomalies[name]) {
      const ct = Date.now() - state.activeAnomalies[name].startTime;
      state.catchTimes.push(ct);
  ...
  function handleWarningClick(warnId) {
    if (!state.running) return;
    if (state.activeWarnings[warnId]) {
      const ct = Date.now() - state.activeWarnings[warnId].startTime;
      state.catchTimes.push(ct);
      state.anomaliesCaught++;
      state.activeWarnings[warnId].caught = true;
      const lightEl = document.querySelector(`.warn-light[data-warn="${warnId}"]`);
      if (lightEl) { lightEl.classList.remove('active'); lightEl.classList.add('acknowledged'); }
  ...
  function updateStats() {
    ...
    const acc = state.anomaliesTotal > 0
      ? Math.round(state.anomaliesCaught / state.anomaliesTotal * 100) + '%'
      : '—';
    $id('im-accuracy').textContent = acc;
  }

  /* ── Warning panel setup ── */
  function buildWarningPanel() {
    const grid = $id('warning-lights');
    if (!grid) return;
    grid.innerHTML = '';
    WARNING_LIGHTS.forEach(w => {
      const div = document.createElement('div');
      div.className = 'warn-light';
      div.dataset.warn = w;
      div.textContent = w;
  ...
  function start() {
    ...
    // Gauge click listeners (use data attribute to avoid duplicate handlers)
  ...
  function endTest() {
    ...
    clearTimeout(anomalyScheduler);
    clearInterval(countdownInterval);
    document.getElementById('status-dot').className = 'status-dot';
    document.getElementById('status-label').textContent = 'READY';

    const accuracy = state.anomaliesTotal > 0
      ? Math.round(state.anomaliesCaught / state.anomaliesTotal * 100)
      : 0;
    const avgCT = state.catchTimes.length > 0
      ? Math.round(state.catchTimes.reduce((a, b) => a + b, 0) / state.catchTimes.length)
      : null;

    ADAPTStorage.addScore('monitoring', {
      accuracy,
      avgCatchTime: avgCT,
      caught: state.anomaliesCaught,
      missed: state.anomaliesMissed,
      total: state.anomaliesTotal
    });

    const rEl = $id('im-result');
    ...
    $id('im-retry-btn').addEventListener('click', () => {
      ...
      $id('im-intro').classList.remove('hidden');
      $id('im-arena').classList.add('hidden');
      $id('im-timer').textContent = '02:00';
      $id('im-timer').className = 'module-timer';
    });
    $id('im-arena').classList.add('hidden');
  ...
  function init() {
    $id('im-start-btn').addEventListener('click', start);
  }
